        ...
        for (let i = 0; i < entireTestingResult.testCases.length; i++) {
            ...
            if (testingResult) {
                ...
                for (let blockParams of settings.tests.blocks) {
                    if (blockParams.executable) this._addResultColumnsIntoBlock(blockParams, blocksValues[blockParams.code], testingResult[blockParams.code]);
                    else blockParams.columns[blockParams.columns.length - 1].colspan += 2;
                }
